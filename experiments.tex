...
The Czech-English setup was based on data and models prepared as a part of a larger
...
including the baseline system and all data not directly produced by the tools evaluated
in this work.}
...
English tokens.
The word-alignments were established on a lemmatized version of this combined corpus using
the \emph{fast-align} tool \citep{dyer:fastalign} and the translation model was created
with two factors on both sides of the corpus:
\begin{enumerate}
  \item token with case determined by supervised truecaser
  \item morphological tag assigned by state-of-art part-of-speech taggers % TODO: More info?
\end{enumerate}
...
The weights of all the components have been tuned using standard MERT implementation
...
The evaluation of BLEU score was done using \texttt{evaluator} tool that is also part of Moses
