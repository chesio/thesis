...
encode any context relevant for phrase translation: often, several short
phrase pairs can be used to achieve the approximately same effect in translation
as the longer phrase pair, that as a consequence can be deemed superfluous and
a viable candidate for pruning.

% Introduce the relent filter.
\citet{ling:relentfilter} presented a phrase table filtering method based on
this observation called \emph{relative entropy filtering}.
Their method first establish relative entropy model that measures how likely
...
probability. Then, it applies this model to phrase table pruning.
...
% Give some practical thresholds to consider.
...

% Compare relent filter to sigfilter.
Technically, the \emph{relative entropy filter} also acts as post-filtering tool,
but is more complex to apply than the \emph{significance filter}: the complete
translation model has to be set up in advance with its weights tuned properly
to ensure the soundness of relative entropy metric.
Because of this prerequisite, time requirements of relative entropy pruning will be
most often far above those of significance pruning as the state of art methods of
translation model tuning are generally very time demanding.
