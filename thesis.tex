...
\emph{RNDr. Ondřej Bojar Ph.D., Ústav formální a aplikované lingvistiky}
...
v~systémech pro statistický strojový překlad, který dokáže odstranit fráze
s~nízkou četností přímo během extrakce tabulky.
Provedená implementace umožňuje vytvářet značně filtrované tabulky, které však
při použití v~překladovém systému umožnují dosáhnout překladu stejné kvality,
Výsledkem této práce je plně funkční program pro extrakci frázových tabulek,
schopný nahradit stávající programy se~stejnou funkčností, jenž jsou součástí
open-source systému pro statistický strojový překlad Moses.
Značná část této práce je věnována experimentálnímu vyhodnocení systémových nároků
programu a kvality frázových tabulek, kde jsou jako trénovací data použity paralelní
korpusy o~celkové velikosti 1~miliardy slov.}
...
\textbf{Supervisor:}
\emph{RNDr. Ondřej Bojar Ph.D., Institute of Formal and Applied Linguistics}
...
The implementation allows for the bulk of extracted phrase pairs to be much reduced with
no significant harm to the ultimate quality of the phrase-based
translation model as measured by a state-of-the-art evaluation measure.
The result of this implementation is a fully working program capable to act
as an alternative to the existing tools for phrase table creation and
filtration that are part of the open-source SMT system Moses.
